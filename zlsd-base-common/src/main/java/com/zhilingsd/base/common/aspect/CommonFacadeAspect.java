...
//    protected Validator validator;
    ...
    public Object before(ProceedingJoinPoint jp) throws Throwable {
        ...
        String assetCompanyType = Optional.ofNullable(request.getHeader("assetCompanyType")).orElseThrow(() -> new ServiceException(ReturnCode.SYSTEM_ERROR.getCode(), "请求头assetCompanyType不能为空"));
        Optional.ofNullable(AssetCompanyTypeEnum.find(assetCompanyType)).orElseThrow(()->new ServiceException(ReturnCode.SYSTEM_ERROR.getCode(), "请求头assetCompanyType值不正确"));
        AppAgentInfo agentInfo = new AppAgentInfo(session,Long.parseLong(operatorId), Long.parseLong(collectionCompanyId), Long.parseLong(collectionGroupId),assetCompanyType);
